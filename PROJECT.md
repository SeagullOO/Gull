...
**设计要点**：
- 主题初始化在 `useEffect` 中通过 `localStorage("gdt_settings")` 读取
...
- 缩放值持久化到 `localStorage("gdt_settings")`
...
- **HMR 安全单例**：通过 `window.__GDT_DB__` 保持 Vite 热更新时复用同一 IndexedDB 连接
...
| 浏览器 | IndexedDB → `GameDesignTool` | Dexie 结构化数据 |
...
通过 `localStorage("gdt_recent_workspaces")` 存储最近 10 个工作区，在 FileExplorer 底部显示。
...
- 持久化：`localStorage("gdt_settings").zoom` 和 `contentZoom`
